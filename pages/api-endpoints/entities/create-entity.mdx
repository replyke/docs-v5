...
Replyke can be integrated on top of existing datasets. Developers can pass the `foreignId` corresponding to the ID of the item in their dataset. Alternatively, a static string can be passed if an entity should be created for a static element, such as `"home-page"`. If no `foreignId` is provided, it will default to the automatically generated entity ID.
...
| Parameter   | Type     | Required | Description |
|------------|----------|----------|-------------|
| `foreignId` | `string` | No | A unique foreign ID for the entity. |
