...
The `useListsData` hook provides a comprehensive and centralized solution for managing hierarchical lists. Unlike individual hooks such as `useCreateList`, `useAddToList`, or `useFetchRootList`, this hook combines all list-related functionalities into a unified interface, withing the context of the logged in user. It enables creating, updating, deleting, and navigating through lists, as well as managing list entities. It is ideal for applications that need to handle complex list structures with parent-child relationships and caching mechanisms for efficient data access.
...
| `subLists`          | `List[]`                              | The sub-lists of the current list.                                           |
...
| `createList`        | `(props: { listName: string }) => Promise<void>` | Creates a new sub-list under the current list.                              |
